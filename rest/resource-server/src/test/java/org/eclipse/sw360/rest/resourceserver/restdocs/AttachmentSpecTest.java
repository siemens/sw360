...
public class AttachmentSpecTest extends TestRestDocsSpecBase {
    ...
    public void should_document_get_attachments_by_sha1() throws Exception {
        String accessToken = TestHelper.getAccessToken(mockMvc, testUserId, testUserPassword);
        MvcResult result  = mockMvc.perform(get("/api/attachments?sha1=da373e491d3863477568896089ee9457bc316783")
                .header("Authorization", "Bearer " + accessToken)
                .accept(MediaTypes.HAL_JSON))
                .andExpect(status().isOk())
                .andDo(this.documentationHandler.document(
                        ...
                        responseFields(
                                fieldWithPath("_links").description("<<resources-index-links,Links>> to other resources"),
                                fieldWithPath("_embedded.sw360:attachments").description("The collection of <<resources-attachment,Attachment resources>>. In most cases the result should contain either on element or an empty collection. If the same binary file is uploaded and attached to multiple sw360 resources, the collection will contain all the attachments with matching sha1 hash.")
                        )))
                .andReturn();
    }

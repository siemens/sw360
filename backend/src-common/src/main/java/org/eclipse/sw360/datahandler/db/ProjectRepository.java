...
public class ProjectRepository extends SummaryAwareRepository<Project> {
    ...
    private static final String MY_ACCESSIBLE_PROJECTS_COUNT =
            ...
            "  if (doc.type == 'project') {\n" +
            "    var acc = {};\n" +
            "    if(doc.createdBy && doc.visbility != \"EVERYONE\")\n" +
            "      acc[doc.createdBy]=1;\n" +
            "    if(doc.leadArchitect && doc.visbility != \"EVERYONE\")\n" +
            "      acc[doc.leadArchitect]=1;\n" +
            "    for(var i in doc.moderators && doc.visbility != \"EVERYONE\") { \n" +
            "      acc[doc.moderators[i]]=1;\n" +
            "    }\n" +
            "    for(var i in doc.contributors && doc.visbility != \"EVERYONE\") {\n" +
            "      acc[doc.contributors[i]]=1;\n" +
            "    }\n" +
            "    if(doc.projectResponsible && doc.visbility != \"EVERYONE\")\n" +
            "      acc[doc.projectResponsible]=1;\n" +
            "    for(var i in acc){\n" +
            "      emit(i,null);\n" +
            "    }\n" +
...
//        return Sets.union(buProjects, myProjects);
    ...
    public int getMyAccessibleProjectsCount(User user) {
        return getConnector().getDocumentCount(Project.class, "myfullprojectscount", new String[] {user.getEmail(), "everyone"});
    }
}

...
public class ProjectDatabaseHandler extends AttachmentAwareDatabaseHandler {
    ...
    private void addCommentToClearingRequest(Project updated, Project current, User user) {
        ...
        StringBuilder releaseChangesSummary = new StringBuilder();
        ...
        if (CommonUtils.isNotEmpty(added)) {
            ...
            releaseChangesSummary.append("<br><b>Added Releases:</b><ul>");
            for (String relName : releaseNames) {
                releaseChangesSummary.append("<li>").append(relName.trim()).append("</li>");
            }
            releaseChangesSummary.append("</ul>");
        }
        if (CommonUtils.isNotEmpty(removed)) {
            ...
            releaseChangesSummary.append("<br><b>Removed Releases:</b><ul>");
            for (String relName : releaseNames) {
                releaseChangesSummary.append("<li>").append(relName.trim()).append("</li>");
            }
            releaseChangesSummary.append("</ul>");
        ...
        if (releaseChangesSummary.length() == 0) {
            releaseChangesSummary.append("<br>No release additions or removals");
        }
        ...
        sendMailForUpdatedProjectWithClearingRequest(updated, releases, nonOssCompCount, compTypeCountMap, user, releaseChangesSummary.toString());
    }
    ...
    private void sendMailForUpdatedProjectWithClearingRequest(Project updated, Collection<Release> releases, String nonOssCompCount, Map<ComponentType, Integer> compTypeCountMap, User user, String releaseChangesSummary) {
        ...
        mailUtil.sendClearingMail(ClearingRequestEmailTemplate.PROJECT_UPDATED, MailConstants.SUBJECT_FOR_UPDATED_PROJECT_WITH_CLEARING_REQUEST,
                ...
                nonOssCompCount, releaseChangesSummary, extractReleaseNameForClearingEmail(releases));
    }

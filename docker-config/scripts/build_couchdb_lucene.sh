...
build_couchdb_lucene() {
  ...
  sed -i "s/dir=indexes/dir=\/app\/liferay-portal-7.2.0-ga1\/tomcat-9.0.17\/bin\/indexes/" ./src/main/resources/couchdb-lucene.ini
  wget https://raw.githubusercontent.com/sw360/sw360vagrant/master/shared/couchdb-lucene.patch
  patch -p1 < couchdb-lucene.patch
  mvn -s /app/build/sw360/docker-config/mvn-proxy-settings.xml clean install war:war

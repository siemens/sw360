...
configure_postgres() {
  ...
  sed -i "s/host    all             all             127.0.0.1\/32            md5/host    all             all             0.0.0.0\/0               md5/" /etc/postgresql/9.5/main/pg_hba.conf
  sed -i "s/#listen_addresses = 'localhost'/listen_addresses = '*'/" /etc/postgresql/9.5/main/postgresql.conf
  /etc/init.d/postgresql restart
}

install_postgres() {
  apt-get install postgresql-9.5 -y --no-install-recommends
}

...
public class UserPortlet extends Sw360Portlet {
    ...
    private Object[] fetchUsersFromDBAndSetRequestAttributes(PortletRequest request, String email) {
        ...
        if (userByEmailFromCouchDB != null) {
            request.setAttribute("userObj", userByEmailFromCouchDB);
    ...
    public void update(ActionRequest request, ActionResponse response)
        ...
        if (CommonUtils.isNotNullEmptyOrWhitespace(email)) {
            ...
            if (userFromCouchAndLiferayDB[1] != null) {
                ...
                try {
                    updateUserObjectFromRequest(request, response, null, userByEmailFromCouchDB, false, false,
                ...
                try {
                    updateUserObjectFromRequest(request, response, new UserCSV(), userByEmailFromCouchDB, false, true,
                            null);
                } catch (TException | SystemException | PortalException e) {
                    setErrorResponse(request, response, e, email);
                    return;
            ...
            try {
                updateUserObjectFromRequest(request, response, userCSV, userByEmailFromCouchDB, true, true, null);
            } catch (TException | SystemException | PortalException e) {
    ...
    private void updateUserObjectFromRequest(ActionRequest request, ActionResponse response, UserCSV userCSV,
            ...
            throws TException, SystemException, PortalException {
        UserService.Iface userClient = thriftClients.makeUserClient();
        ...
        if (isLiferayUserNew) {
            if (!emailFromReq.equals(userByEmailFromCouchDB.getEmail())) {
                try {
                    userClient.getByEmail(emailFromReq);
                ...
                if (!externalIdFromReq.equals(userByEmailFromCouchDB.getExternalid())) {
                    try {
                        userClient.getByEmailOrExternalId("dummy@dummy.org", externalIdFromReq);
    ...
    private void prepareUserEdit(RenderRequest request) {
        ...
        if (email != null) {
            ...
            request.setAttribute("userObj", new org.eclipse.sw360.datahandler.thrift.users.User());
        }
    ...
    private JSONArray getUserData(List<org.eclipse.sw360.datahandler.thrift.users.User> userList,
        ...
        for (int i = start; i < start + paginationParameters.getDisplayLength(); i++) {
            if (i == userList.size()) {
                break;
            }
